# Source: https://github.com/kevinmcaleer/vl53l0x
# Kevin McAleer, March 2021
...
class VL53L0X():
    def __init__(self, i2c, address=0x29):
        ...
        wait = asyncio.run(async_sleep_ms(100))
        self.init()
        self._started = False
    ...
    def _register(self, register, value=None, struct='B'):
        if value is None:
            return self._registers(register, struct=struct)[0] # type: ignore
    ...
    def _flag(self, register=0x00, bit=0, value=None):
        ...
        mask = 1 << bit
        if value is None:
            return bool(data & mask) # type: ignore
        elif value:
            data |= mask # type: ignore
        else:
            data &= ~mask # type: ignore
    ...
    def init(self, power2v8=True):
        ...
        spad_map = bytearray(self._registers(_SPAD_ENABLES, struct='6B')) # type: ignore
    ...
    def _spad_info(self):
        ...
        for timeout in range(_IO_TIMEOUT):
            if self._register(0x83):
                break
            wait = asyncio.run(async_sleep_ms(1))
        else:
            raise TimeoutError()
        self._config(
            (0x83, 0x01),
        ...
        count = value & 0x7f # type: ignore
        is_aperture = bool(value & 0b10000000) # type: ignore
    ...
    def get_sequence_step_enables(self):
        ...
        self.enables["tcc"] = (sequence_config >> 4) & 0x1 # type: ignore
        self.enables["dss"] = (sequence_config >> 3) & 0x1 # type: ignore
        self.enables["msrc"] = (sequence_config >> 2) & 0x1 # type: ignore
        self.enables["pre_range"] = (sequence_config >> 6) & 0x1 # type: ignore
        self.enables["final_range"] = (sequence_config >> 7) & 0x1 # type: ignore
    ...
    def get_sequence_step_timeouts(self):
        ...
        self.timeouts["msrc_dss_tcc_mclks"] = int(self._register(MSRC_CONFIG_TIMEOUT_MACROP)) + 1 # type: ignore
    ...
    def perform_single_ref_calibration(self, vhv_init_byte):
        ...
        while self._register((RESULT_INTERRUPT_STATUS & 0x07) == 0):
            ...
            wait = asyncio.run(async_sleep_ms(0))
